import {
    firebaseLogout as firebaseLogout,
    initializeFirebase,
    firebaseSignInWithFacebook,
    firebaseSignInUserEmailNPass,
    firebaseCreateUserEmailNPass,
    firebaseSignInWithGoogle,
    firebaseSendPasswordResetLink,
    firebaseGetConfirmEmailLink,
    firebaseAfterConfirmApplyCode,
    firebaseChangePassword,
} from "./auth_firebase.js";
...
const { loading, unauthorized, authorized, emailNotConfirm } =
    authSlice.actions;
...
const confirmLoginPromise = async (callback = () => { }) => {
    ...
    let prom = new Promise(
        (resolve, reject) =>
        (cancel = __authHandle.onAuthStateChanged(
            (user) => {
                if (user?.email) {
                    resolve(user);
                } else {
...
export const getAuthUser = () => {
    ...
    return JSON.parse(currentState.authUser);
};
